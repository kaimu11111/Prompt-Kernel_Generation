...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        // Load B into shared memory (transposed column-major)
        ...
        // Unroll computation by 8 using FMA for better ILP
        #pragma unroll
        for (int k = 0; k < TILE_WIDTH; k += 8) {
            sum = __fma_rn(shared_A[ty][k], shared_B[k][tx], sum);
            sum = __fma_rn(shared_A[ty][k+1], shared_B[k+1][tx], sum);
            sum = __fma_rn(shared_A[ty][k+2], shared_B[k+2][tx], sum);
            sum = __fma_rn(shared_A[ty][k+3], shared_B[k+3][tx], sum);
            sum = __fma_rn(shared_A[ty][k+4], shared_B[k+4][tx], sum);
            sum = __fma_rn(shared_A[ty][k+5], shared_B[k+5][tx], sum);
            sum = __fma_rn(shared_A[ty][k+6], shared_B[k+6][tx], sum);
            sum = __fma_rn(shared_A[ty][k+7], shared_B[k+7][tx], sum);
        }
    }
