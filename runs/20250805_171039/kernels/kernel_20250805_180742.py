...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        // Load B into shared memory (column-major)
        int b_col = col;
        ...
        // Unroll inner loop by 4 and use FMA
        for (int k = 0; k < TILE_WIDTH; k += 4) {
            ...
            // FMA implementation with unrolling
            sum = fma(shared_A[ty][k0], shared_B[tx][k0], sum);
            if (k1 < TILE_WIDTH) sum = fma(shared_A[ty][k1], shared_B[tx][k1], sum);
            if (k2 < TILE_WIDTH) sum = fma(shared_A[ty][k2], shared_B[tx][k2], sum);
            if (k3 < TILE_WIDTH) sum = fma(shared_A[ty][k3], shared_B[tx][k3], sum);
        }

        // Warp-level synchronization for fused FMA reductions
        #pragma unroll
