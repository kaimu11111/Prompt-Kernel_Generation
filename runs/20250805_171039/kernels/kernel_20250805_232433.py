...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    __shared__ float shared_B[TILE_WIDTH][2*TILE_WIDTH]; // Corrected shared_B dimensions
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        shared_B[ty][b_col] = b_valid ? __ldg(&B[b_row * N + block_col + b_col]) : 0.0f;
