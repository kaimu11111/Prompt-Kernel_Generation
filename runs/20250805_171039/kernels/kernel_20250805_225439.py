...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        // 4-way unrolled FMA computation with __fma_rn
        #pragma unroll
...
matmul_optimized = load_inline(
    name="optimized_matmul_v3",
    cuda_sources=source,
    cpp_sources=cpp_src,
    functions=["optimized_matmul_cuda"],
    verbose=True,
    extra_cuda_cflags=["-O3", "--use-fast-math", "--expt-relaxed-constexpr", "--maxrregcount=255"]
)

class ModelNew(nn.Module):
    def __init__(self):
        super().__init__()
        self.matmul_optimized = matmul_optimized.optimized_matmul_cuda  # Store the function directly
