...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    int tx = threadIdx.y; // Bank conflict mitigation via index swap
    int ty = threadIdx.x;
    int bx = blockIdx.x;
    int by = blockIdx.y;
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        // Fully unrolled 8-way FMA computation
        #pragma unroll
        ...
        __syncthreads(); // Only after tile load
    }
...
matmul_optimized = load_inline(
    ...
    verbose=True,
    extra_cuda_cflags=["-O3", "--use_fast_math", "--maxrregcount=255", "--use_fast_math", "--expt-relaxed-constexpr"]
)
