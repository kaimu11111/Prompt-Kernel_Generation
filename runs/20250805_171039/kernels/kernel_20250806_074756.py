...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    __shared__ float shared_A[TILE_WIDTH][TILE_WIDTH + 1]; // Column-padded for row-major A
    __shared__ float shared_B[TILE_WIDTH + 1][TILE_WIDTH]; // Row-padded for column-major B

    int tx = threadIdx.y; // Swap for bank conflict mitigation
    int ty = threadIdx.x;
    int bx = blockIdx.x;
    int by = blockIdx.y;
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        // Load A with __ldg and column padding
        int a_col = m * TILE_WIDTH + tx;
        int a_row = row;
        bool a_valid = (a_row < M) && (a_col < K);
        shared_A[ty][tx] = a_valid ? __ldg(&A[a_row * K + a_col]) : 0.0f;

        // Load B in column-major with __ldg and row padding
        int b_row = m * TILE_WIDTH + tx; // Fixed: use tx instead of ty
