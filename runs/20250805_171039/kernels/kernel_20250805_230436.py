...
__global__ void matmul_optimized_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        // Transpose B to column-major in shared memory with correct indices
        ...
        // Optimized FMA with __fma_rn and 4-way unrolling
        #pragma unroll
        ...
        __syncthreads(); // Added synchronization after each tile iteration
    }
