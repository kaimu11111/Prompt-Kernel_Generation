...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    int tx = threadIdx.y; // Swap indices to mitigate bank conflicts
    int ty = threadIdx.x;
    int bx = blockIdx.x;
    int by = blockIdx.y;
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        // 8-way unrolled FMA with fused operations
        #pragma unroll
