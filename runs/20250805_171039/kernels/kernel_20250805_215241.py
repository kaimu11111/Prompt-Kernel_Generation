...
__global__ void matmul_optimized_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        // 4-way unrolled FMA computation
        #pragma unroll
        for (int k = 0; k < TILE_WIDTH; k += 4) {
            int k0 = k, k1 = k+1, k2 = k+2, k3 = k+3;
            sum += shared_A[ty][k0] * shared_B[k0][tx];
            if (k1 < TILE_WIDTH) sum += shared_A[ty][k1] * shared_B[k1][tx];
            if (k2 < TILE_WIDTH) sum += shared_A[ty][k2] * shared_B[k2][tx];
            if (k3 < TILE_WIDTH) sum += shared_A[ty][k3] * shared_B[k3][tx];
        }
        __syncthreads();  // Added missing syncthreads after shared memory loads
    }
...
class ModelNew(nn.Module):
    def __init__(self):
        super().__init__()
        self.matmul_optimized = matmul_optimized.matmul_optimized_cuda  # Access the actual function
