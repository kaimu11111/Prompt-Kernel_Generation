...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    __shared__ float shared_B[TILE_WIDTH + 1][TILE_WIDTH]; // Add padding to reduce bank conflicts
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        shared_B[tx][ty] = b_valid ? __ldg(&B[b_row * N + b_col]) : 0.0f; // Corrected column-major storage
...
matmul_optimized = load_inline(
    name="matmul_optimized_v7",
    cuda_sources=source,
    cpp_sources=cpp_src,
    functions=["optimized_matmul_cuda"],
    verbose=True,
    extra_cuda_cflags=["-O3", "--use_fast_math", "--expt-relaxed-constexpr", "--maxrregcount=63"]  # Reduced register limit to avoid SM overflow
)
