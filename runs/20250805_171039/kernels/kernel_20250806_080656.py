...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        // Load A into shared memory with column padding using __ldg
        int a_col = m * TILE_WIDTH + tx;
        int a_row = row;
        bool a_valid = (a_row < M) && (a_col < K);
        shared_A[ty][tx] = a_valid ? __ldg(&A[a_row * K + a_col]) : 0.0f;

        // Load B into shared memory in column-major layout with row padding
        ...
        // 8-way unrolled FMA with fused multiply-add and bank conflict-free access
        #pragma unroll
        ...
        __syncthreads();  // Added to ensure sync between tile iterations
    }
...
matmul_optimized = load_inline(
    name="matmul_optimized_v15",
    cuda_sources=source,
    cpp_sources=cpp_src,
    functions=["optimized_matmul_cuda"],
    verbose=True,
    extra_cuda_cflags=["-O3", "--use_fast_math", "--maxrregcount=255"]
)
