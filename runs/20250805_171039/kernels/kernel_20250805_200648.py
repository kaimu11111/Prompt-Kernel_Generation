...
__global__ void matmul_optimized_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        for (int k = 0; k < TILE_WIDTH; k += 4) {
            ...
            sum = fmaf(shared_A[ty][k0], shared_B[k0][tx], sum);
            if (k1 < TILE_WIDTH) sum = fmaf(shared_A[ty][k1], shared_B[k1][tx], sum);
            if (k2 < TILE_WIDTH) sum = fmaf(shared_A[ty][k2], shared_B[k2][tx], sum);
            if (k3 < TILE_WIDTH) sum = fmaf(shared_A[ty][k3], shared_B[k3][tx], sum);
        }
