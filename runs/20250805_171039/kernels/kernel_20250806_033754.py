...
__global__ void matmul_optimized_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    __shared__ float shared_A[TILE_WIDTH][TILE_WIDTH + 1]; // Row-major with column padding
    __shared__ float shared_B[TILE_WIDTH + 1][TILE_WIDTH]; // Column-major with row padding
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        // Load A into shared memory (row-major with padding)
        ...
        // 8-way unrolled FMA with __fma_rn
        #pragma unroll
