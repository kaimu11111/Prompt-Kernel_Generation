...
__global__ void matmul_optimized_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        // Load A into shared memory with row-major access
        int a_col = m * TILE_WIDTH + tx;
        int a_row = row;
        bool a_valid = (a_row < M) && (a_col < K);
        shared_A[ty][tx] = a_valid ? __ldg(&A[a_row * K + a_col]) : 0.0f;

        // Load B into shared memory with column-major transposed layout
        ...
        // Optimized FMA with 4x unrolling
        #pragma unroll
        for (int k = 0; k < TILE_WIDTH; k += 4) {
            ...
            sum = __fmaf__(shared_A[ty][k0], shared_B[k0][tx], sum);
            if (k1 < TILE_WIDTH) sum = __fmaf__(shared_A[ty][k1], shared_B[k1][tx], sum);
            if (k2 < TILE_WIDTH) sum = __fmaf__(shared_A[ty][k2], shared_B[k2][tx], sum);
            if (k3 < TILE_WIDTH) sum = __fmaf__(shared_A[ty][k3], shared_B[k3][tx], sum);
        }

        __syncthreads(); // Only after shared memory operations
    }
