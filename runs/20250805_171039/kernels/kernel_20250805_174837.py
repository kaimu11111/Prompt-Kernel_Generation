...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        for (int k = 0; k < TILE_WIDTH; k += 4) {
            ...
            if (k1 < TILE_WIDTH) sum = __fma__(shared_A[ty][k1], shared_B[tx][k1], sum);
            if (k2 < TILE_WIDTH) sum = __fma__(shared_A[ty][k2], shared_B[tx][k2], sum);
            if (k3 < TILE_WIDTH) sum = __fma__(shared_A[ty][k3], shared_B[tx][k3], sum);
        }
