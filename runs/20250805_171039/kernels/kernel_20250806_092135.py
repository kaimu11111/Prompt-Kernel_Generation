...
__global__ void matmul_optimized_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    __shared__ float shared_A[TILE_WIDTH][TILE_WIDTH + 1] __align__(16);
    __shared__ float shared_B[TILE_WIDTH + 1][TILE_WIDTH] __align__(16);

    int tx = threadIdx.y; // Swap to mitigate bank conflicts
    int ty = threadIdx.x;
    int bx = blockIdx.x;
    int by = blockIdx.y;
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        __syncthreads(); // Ensure sync after tile processing
    }
...
matmul_optimized = load_inline(
    name="matmul_optimized_final",
    cuda_sources=source,
    cpp_sources=cpp_src,
    functions=["matmul_optimized_cuda"],
    verbose=True,
    extra_cuda_cflags=["-O3", "--use_fast_math", "--maxrregcount=255", "--expt-relaxed-constexpr"]
)
