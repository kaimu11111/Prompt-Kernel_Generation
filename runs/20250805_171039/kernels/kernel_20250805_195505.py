...
__global__ void matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        // Load B into shared memory (transposed to column-major)
        ...
        // FMA with unrolled loops
        #pragma unroll
        for (int k = 0; k < TILE_WIDTH; k += 4) {
            int k0 = k, k1 = k + 1, k2 = k + 2, k3 = k + 3;
            sum = fma(shared_A[ty][k0], shared_B[k0][tx], sum);
            if (k1 < TILE_WIDTH) sum = fma(shared_A[ty][k1], shared_B[k1][tx], sum);
            if (k2 < TILE_WIDTH) sum = fma(shared_A[ty][k2], shared_B[k2][tx], sum);
            if (k3 < TILE_WIDTH) sum = fma(shared_A[ty][k3], shared_B[k3][tx], sum);
        }
...
torch::Tensor matmul_cuda(torch::Tensor A, torch::Tensor B) {
    ...
    matmul_kernel<<<grid, threads>>>(C.data_ptr<float>(), 
                                    A.data_ptr<float>(), 
                                    B.data_ptr<float>(), 
                                    M, N, K_A);
