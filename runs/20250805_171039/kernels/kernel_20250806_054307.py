...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    __shared__ float shared_A[TILE_WIDTH][TILE_WIDTH + 1]; // Column padding for row-major A
    __shared__ float shared_B[TILE_WIDTH + 1][TILE_WIDTH]; // Row padding for column-major B
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        // Load B into shared memory in column-major format with __ldg
        int b_row = m * TILE_WIDTH + ty;
        int b_col = block_col + tx;
        bool b_valid = (b_row < K) && (b_col < N);
        // Fixed B's index calculation for column-major access
        ...
        __syncthreads();  // Moved sync to ensure tiles are processed independently
    }
