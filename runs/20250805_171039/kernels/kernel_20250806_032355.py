...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    __shared__ float shared_A[TILE_WIDTH][TILE_WIDTH + 1]; // Row-major A with column padding
    __shared__ float shared_B[TILE_WIDTH + 1][TILE_WIDTH]; // Column-major B with row padding
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        // Load A into shared memory with row-major and __ldg
        int a_col = m * TILE_WIDTH + tx;
        int a_row = row;
        bool a_valid = (a_row < M) && (a_col < K);
        shared_A[ty][tx] = a_valid ? __ldg(&A[a_row * K + a_col]) : 0.0f;

        // Load B into shared memory with column-major (transposed storage)
        int b_row = m * TILE_WIDTH + ty;
        int b_col = block_col + tx;
        bool b_valid = (b_row < K) && (b_col < N);
        shared_B[ty][tx] = b_valid ? __ldg(&B[b_row * N + b_col]) : 0.0f; // Fixed index order

        __syncthreads();

        // 8-way unrolled FMA computation with __fma_rn and register blocking
        #pragma unroll
