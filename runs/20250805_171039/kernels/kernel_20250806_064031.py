...
__global__ void matmul_optimized_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    int tx = threadIdx.y; // Mitigate bank conflicts via swapped indices
    int ty = threadIdx.x;
    int bx = blockIdx.x;
    int by = blockIdx.y;
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        // Load B with __ldg and row padding (column-major layout)
        int b_row = m * TILE_WIDTH + ty;
        int b_col = block_col + tx;
        bool b_valid = (b_row < K) && (b_col < N);
        shared_B[ty][tx] = b_valid ? __ldg(&B[b_col * K + b_row]) : 0.0f; // Fixed column-major access
