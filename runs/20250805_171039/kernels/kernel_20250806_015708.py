...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    __shared__ float shared_A[TILE_WIDTH + 1][TILE_WIDTH]; // Padded first dimension for bank conflict reduction
    __shared__ float shared_B[TILE_WIDTH + 1][TILE_WIDTH]; // Padded first dimension for column-major storage
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        shared_B[tx][ty] = b_valid ? __ldg(&B[b_row * N + b_col]) : 0.0f; // Transposed storage
...
torch::Tensor optimized_matmul_cuda(torch::Tensor A, torch::Tensor B) {
    ...
    // Ensure contiguous memory layout for __ldg
