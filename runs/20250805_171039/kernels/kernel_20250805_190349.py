...
__global__ void optimized_matmul_kernel(float* C, const float* A, const float* B, int M, int N, int K) {
    ...
    for (int m = 0; m < (K + TILE_WIDTH - 1) / TILE_WIDTH; m++) {
        ...
        // Load B into shared_B (column-major transposed)
        int b_col = col;
        ...
        // Unroll loop by 4 with FMA and #pragma unroll directive
        #pragma unroll
        for (int k = 0; k < TILE_WIDTH; k += 4) {
            ...
            sum = fmaf(shared_A[ty][k0], shared_B[tx][k0], sum);
            if (k1 < TILE_WIDTH) sum = fmaf(shared_A[ty][k1], shared_B[tx][k1], sum);
            if (k2 < TILE_WIDTH) sum = fmaf(shared_A[ty][k2], shared_B[tx][k2], sum);
            if (k3 < TILE_WIDTH) sum = fmaf(shared_A[ty][k3], shared_B[tx][k3], sum);
        }
...
class ModelNew(nn.Module):
    ...
    def forward(self, A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
        return self.matmul_optimized.optimized_matmul_cuda(A, B)
