        ...
        for (int dx = 0; dx < input_region_width; dx++) {
            for (int dy = 0; dy < input_region_height; dy++) {
                ...
                input_shared[shared_idx] = (global_x >= 0 && global_x < input_width &&
                                           global_y >= 0 && global_y < input_height) ? input[input_idx] : 0;
            }
        }
    }
...
torch::Tensor conv_relu_add_bias_cuda(torch::Tensor input,
                                     torch::Tensor weight,
                                     torch::Tensor bias) {
    ...
    AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "conv_relu_add_bias_cuda", ([&] {
...
conv_relu_add_bias = load_inline(
    name="conv_relu_add_bias",
    cuda_sources=source,
    cpp_sources=cpp_src,
    functions=["conv_relu_add_bias_cuda"],
    verbose=True
)
...
class ModelNew(nn.Module):
    ...
    def forward(self, x):
        return conv_relu_add_bias.conv_relu_add_bias_cuda(x, self.weight, self.bias)
