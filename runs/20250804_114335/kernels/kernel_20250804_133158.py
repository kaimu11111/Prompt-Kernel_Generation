...
__global__ void matmul_optimized(const float *A, const float *B, float *C, int M, int N, int K) {
    ...
    for (int kStart = 0; kStart < K; kStart += TILE_WIDTH) {
        // Determine current/next shared memory buffers
        ...
        // Load current tile into shared memory with coalesced access
        ...
        // Preload next tile if applicable
        ...
        // Unrolled computation with better ILP
        #pragma unroll
        for (int i = 0; i < TILE_WIDTH; ++i) {
            sum += current_sA[tx][i] * current_sB[i][ty];
        }

        // Toggle buffer and synchronize
        if (threadIdx.x == 0 && threadIdx.y == 0) {
            swap_flag = !swap_flag;
        }
        __syncthreads();
...
// Host wrapper with optimized configuration
...
matmul_op = load_inline(
    ...
    verbose=True,
    extra_cuda_cflags=["-std=c++17", "-Xptxas=-dlcm=ca"]
)
