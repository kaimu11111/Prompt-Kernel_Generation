...
__global__ void matmul_optimized(const float *A, const float *B, float *C, int M, int N, int K) {
    __shared__ __align__(16) float sA[TILE_WIDTH][TILE_WIDTH];
    __shared__ __align__(16) float sB[TILE_WIDTH][TILE_WIDTH];
    ...
    for (int kStart = 0; kStart < K; kStart += TILE_WIDTH) {
        ...
        __syncthreads(); // Added missing barrier after unrolled computation
    }
