...
__global__ void optimized_relu(const float* x, float* y, int n) {
    ...
    // Process full chunks with individual element loads
    for (int i = tid; i < num_full; i += blockDim.x * gridDim.x) {
        int idx = i *4;
        y[idx] = fmaxf(x[idx], 0.0f);
        y[idx+1] = fmaxf(x[idx+1], 0.0f);
        y[idx+2] = fmaxf(x[idx+2], 0.0f);
        y[idx+3] = fmaxf(x[idx+3], 0.0f);
    }
...
torch::Tensor optimized_relu_cuda(torch::Tensor x) {
    ...
    if (grid_size == 0) grid_size = 1; // Prevent zero grid size
