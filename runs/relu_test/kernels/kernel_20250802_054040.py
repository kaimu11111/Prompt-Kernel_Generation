...
__global__ void optimized_relu(const float* x, float* y, int n) {
    ...
    // Load chunks into shared memory using vectorized __ldg
    ...
    for (int i = tid; i < (block_end - block_start); i += block_size) {
        reinterpret_cast<float4*>(y)[block_start + i] = shared[i];
    }

    // Handle residual elements with __ldg and grid-strided
    if (residual > 0) {
        int residual_start = num_full *4;
        int global_tid = bid * block_size + tid;
        for (int i = global_tid; i < residual; i += blockDim.x * gridDim.x) {
...
extern "C" {
    __global__ void optimized_relu(const float*, float*, int);
    void optimized_relu_forward(const at::Tensor x, at::Tensor y) {
        int n = x.numel();
        int num_full = n /4;
