...
__global__ void optimized_relu(const float* x, float* y, int n) {
    ...
    // Load chunks into shared memory with __ldg for optimized global memory access
    ...
    // Process elements in shared memory to reduce latency
    for (int i = tid; i < (block_end - block_start); i += block_size) {
        float4 val = shared[i];
    ...
    // Write back to global memory using coalesced accesses
    for (int i = tid; i < (block_end - block_start); i += block_size) {
        int global_idx = block_start + i;
        reinterpret_cast<float4*>(y)[global_idx] = shared[i];
    }

    // Handle residual elements without divergence using grid-strided loop
    if (residual > 0) {
